...
function ProjectWizard() {
    ...
    const [spec, setSpec] = (0, react_1.useState)(null);
    ...
    return (<div className="bg-white shadow-2xl rounded-3xl overflow-hidden border border-gray-100">
      <div className="p-8 sm:p-12">
        {step === 1 && (<div className="space-y-6">
            ...
            <textarea className="w-full h-48 p-4 text-lg border-2 border-gray-200 rounded-2xl focus:border-black focus:ring-0 transition-colors resize-none" placeholder="e.g. A marketplace for vintage collectibles with a built-in auction system..." value={pitch} onChange={(e) => setPitch(e.target.value)}/>
            <button onClick={generateSpec} className="w-full bg-black text-white text-xl font-bold py-4 rounded-2xl hover:bg-gray-800 transition-all transform hover:scale-[1.02] active:scale-[0.98]">
              Generate Blueprint
            </button>
          </div>)}

        {step === 2 && (<div className="flex flex-col items-center justify-center py-12 space-y-6">
        ...
        {step === 3 && spec && (<div className="space-y-8">
            ...
            <div className="flex gap-4">
              <button onClick={() => setStep(1)} className="flex-1 border-2 border-gray-200 text-gray-600 font-bold py-4 rounded-2xl hover:bg-gray-50 transition-all">
                Refine Pitch
              </button>
              <button onClick={deploy} className="flex-1 bg-black text-white font-bold py-4 rounded-2xl hover:bg-gray-800 transition-all shadow-lg">
                Build & Deploy
              </button>
            </div>
          </div>)}

        {step === 4 && (<div className="flex flex-col items-center justify-center py-12 space-y-6">
        ...
        {step === 5 && (<div className="text-center py-12 space-y-6">
